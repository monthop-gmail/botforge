...
async function sendPrompt(
  ...
  if (!session) {
    ...
    sessions.set(userId, { sessionId: created.id })
...
async function handleTextMessage(
  ...
  if (text.toLowerCase() === "/about" || text.toLowerCase() === "/who") {
    await lineClient.replyMessage({
      ...
      messages: [{
        type: "text",
        text: `Qwen Code LINE Bot

AI Coding Agent powered by Qwen Code CLI
...
Just type your message to chat with AI`,
  ...
  enqueueForUser(userId, async () => {
    try {
      const { result, isError } = await sendPrompt(userId, text)
