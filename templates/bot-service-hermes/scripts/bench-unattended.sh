...
run_trials() {
  ...
  for i in $(seq 1 "$n"); do
    ...
    # 🔴 ล้างกล่องงานก่อน — contamination อีกรูปที่ harness เคยพลาด
    #    fixture ของรอบก่อนที่ค้างเป็น in_progress จะไปโผล่ใน waiting_for_you
    #    ของรอบถัดมา แล้ว agent เห็นงานหลายใบพร้อมกันจนหยุดถามว่าจะรับใบไหน
    #    (เจอจริง: "พบว่ามี 2 ใบงานที่ยังไม่รับ ... จะรับทั้งคู่ไหมครับ?")
    for stale in $(mcp "monthop-gmail/botforge" get_tasks "{\"assigned_to\":\"$NAME\",\"status\":\"in_progress\",\"limit\":20}" | jq -r '.tasks[]?.id'); do
      mcp "monthop-gmail/botforge" update_task "{\"task_id\":\"$stale\",\"status\":\"done\",\"detail\":\"ปิดอัตโนมัติก่อนรอบวัดใหม่ — fixture ค้างจากรอบก่อน ไม่ใช่งานเสร็จ\"}" >/dev/null
    done

    # 🔴 กระทู้ใหม่ต่อรอบ — จุดที่แก้ contamination ของ harness เดิม
    #    ของเดิมให้ทุกรอบโพสต์ลงกระทู้เดียวกัน รอบที่ n จึงเห็นผลงานของรอบ 1..n-1
    #    แล้วสรุปว่า "งานนี้ทำไปแล้ว" — agent พูดเองสามรอบ (dis-6c9dd6e3 seq 32)
    #    รอบนี้แต่ละรอบมีที่ทิ้งผลของตัวเอง ไม่เห็นของใคร
    local disc
    disc=$(mcp "monthop-gmail/botforge" create_discussion \
      "{\"title\":\"[BENCH-$$-$i] ที่ทิ้งผลของรอบวัดนี้รอบเดียว\",\"kind\":\"note\",\"body\":\"กระทู้นี้มีไว้ให้รอบวัดที่ $i โพสต์ผลลงมาที่เดียว · ไม่ใช่กระทู้สำหรับคุยกัน · สร้างใหม่ทุกรอบโดยตั้งใจ เพื่อไม่ให้รอบไหนเห็นผลงานของรอบอื่น\"}" \
      | jq -r '.discussion_id')
    [ "$disc" = null ] && { echo "$i: สร้างกระทู้ไม่ได้"; continue; }

    task=$(mcp "monthop-gmail/botforge" create_task \
      "{\"title\":\"[BENCH $i] รอบวัดความสม่ำเสมอ — ใช้แล้วทิ้ง\",\"assigned_to\":\"$NAME\",\"discussion_id\":\"$disc\",\"detail\":\"fixture ของการวัด ไม่ใช่งานจริง · รับใบ อ่านสดจาก server โพสต์ข้อความสั้นหนึ่งข้อความลงกระทู้ $disc ว่ารอบที่ $i ทำงานแล้ว แล้วปิดใบนี้เป็น done หลังโพสต์ปรากฏ · โพสต์ผลของรอบนี้เสมอ แม้กระทู้จะมีข้อความอื่นอยู่แล้วก็ตาม · ใช้ tool ใน allowlist เท่านั้น\"}" \
      | jq -r '.task_id')
    [ "$task" = null ] && { echo "$i: สร้าง task ไม่ได้"; continue; }
    ho=$(mcp "monthop-gmail/botforge" create_handoff \
      "{\"task_id\":\"$task\",\"to\":\"$NAME\",\"context\":\"รอบวัดที่ $i · รับใบ อ่านสดจาก server โพสต์ข้อความสั้นลงกระทู้ $disc แล้วปิดใบหลังโพสต์ปรากฏ · โพสต์ผลของรอบนี้เสมอ แม้กระทู้จะมีข้อความอื่นอยู่แล้ว · ทำให้ครบในรอบเดียว ไม่ต้องรอคำสั่งเพิ่ม · ถ้าติดจริงให้โพสต์ blocker สั้น ๆ\"}" \
      | jq -r '.handoff_id')
