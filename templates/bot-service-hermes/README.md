...
## ต่อกับ ai-collaboration-mcp

เทมเพลตนี้ต่อกับโต๊ะกลางของทีมได้ แยกเป็นสองทางที่เปิดปิดคนละอัน

| | ทำอะไรได้ | ใบที่ใช้ | ค่าเริ่มต้น |
| --- | --- | --- | --- |
| `ai-collab` | อ่าน 4 อย่าง — งานที่รอ, task, handoff, กระทู้ | ใบไหนก็ได้ | ปิด |
| `ai-collab-write` | รับงาน + รายงานผล + ปิด task | **ต้องเป็นใบของ instance เอง** | ปิด |

แยกกันเพราะ Hermes ตั้งชื่อ env จากชื่อ server ให้เอง จึงถือคนละใบได้ —
เปิดอ่านไว้โดยที่เขียนยังปิดสนิท และถอนสิทธิ์เขียนได้ด้วยการลบใบเดียว

### ชื่อที่ใช้ประกาศตัว

`AI_COLLAB_CLIENT_NAME` ต้องเป็นชื่อของ **instance** ไม่ใช่ของ botforge

botforge เป็นโรงงานที่ออก instance ได้หลายตัว ถ้าทุกตัวประกาศตัวเป็นชื่อโรงงาน
งานที่ส่งถึงตัวหนึ่งจะโผล่ที่ทุกตัว และย้อนดูทีหลังจะแยกไม่ออกว่าใครทำ

วัดจริงแล้วว่าแยกกันจริง ไม่ใช่แค่ป้ายชื่อ — โทเค็นใบเดียวกัน เปลี่ยนแค่ header:

```
X-Client-Name: monthop-gmail/nst-hermes  →  เห็นเฉพาะงานของ instance
X-Client-Name: monthop-gmail/botforge    →  เห็นเฉพาะงานของโรงงาน
```

### คำสั่ง

```bash
./scripts/collab-inbox.sh            # งานที่รออยู่
./scripts/collab-inbox.sh --gates    # ตรวจว่าเปิดสิทธิ์เขียนได้ยัง
./scripts/collab-inbox.sh --plan     # จะเรียกอะไรบ้างถ้ารับงาน (ไม่ยิงจริง)
```

`--gates` ตรวจให้ห้าข้อ รวมถึงข้อที่คนมักข้าม คือ **ใบที่ใช้เขียนต้องไม่ใช่ใบเดียว
กับใบที่ใช้อ่าน** — ถ้าเป็นใบเดียวกันแปลว่าสิทธิ์เขียนวิ่งด้วยตัวตนที่ปลอมได้

### กฎที่บังคับไว้ในไฟล์ ไม่ใช่แค่เขียนบอก

**สิทธิ์เขียนมีเท่าที่ lifecycle ของงานบังคับ** — รับงาน, รายงาน, ปิด task
ไม่มี `create_task` / `create_handoff` / `record_decision` เพราะนั่นคือการ
*สั่งงาน* ไม่ใช่ *ทำงานที่ถูกสั่ง*

**หากาจากกล่องของตัวเอง ไม่ใช่จาก `get_handoffs`** — `get_handoffs` เป็นการค้น
ทั้งโต๊ะ ใครเรียกก็เห็นเท่ากัน (วัดแล้ว) ส่วน `waiting_for_you` ใน
`get_workspace_context` ต่างหากที่ผูกกับตัวตนของผู้เรียก

**รายงานผลต้องชี้หลักฐานได้** — สิ่งที่ตัวเองจำว่าทำ ไม่นับเป็นหลักฐาน
ต้องชี้ของที่ตรวจซ้ำได้ เช่นผลของเครื่องมือ, output ของเทสต์, commit
ถ้าไม่มีหลักฐาน หรือหลักฐานขัดกับที่จะรายงาน ให้รายงานว่าติด ไม่ใช่ว่าเสร็จ

เหตุผลของข้อสุดท้ายไม่ใช่ความระวังลอย ๆ — โมเดลรายงานว่าตัวเองเรียกเครื่องมือ
ที่ไม่เคยถูกเรียกได้จริง เจอมาแล้วในโปรเจกต์นี้
