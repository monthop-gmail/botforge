...
console.log("Qwen Code LINE bot configuration:")
...
function isBotMentioned(event: any): boolean {
  ...
  if (text.startsWith("@bot") || text.startsWith("qwen") || text.startsWith("@qwen")) return true
...
async function handleTextMessage(
  ...
  if (text.toLowerCase() === "/about" || text.toLowerCase() === "/who") {
    const aboutMsg = `สวัสดีครับ! ผมคือ Qwen Code Bot

AI Coding Agent (Qwen Code CLI)
...
Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (req.method === "GET" && url.pathname === "/") {
      return new Response("Qwen Code LINE Bot is running")
    }
...
<title>Qwen Code LINE Bot — About</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  h1{font-size:1.8rem;margin-bottom:.5rem;background:linear-gradient(135deg,#6366f1,#8b5cf6);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
  h2{font-size:1.3rem;margin-top:2rem;margin-bottom:.75rem;color:#6366f1}
...
<body>
<div class="container">
  <h1>Qwen Code LINE Bot</h1>
  <p>AI coding assistant powered by Qwen Code CLI — ถามคำถาม ให้ AI ช่วยตอบได้เลยผ่าน LINE</p>
  ...
  <footer><p>Built with Qwen Code CLI + LINE Messaging API</p></footer>
</div>
</body>
</html>`
...
log(
  `Qwen Code LINE bot listening on http://localhost:${port}/webhook`,
)
