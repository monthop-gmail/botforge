...
import { runCodexAppServer as runCodex } from "./codex-appserver"
...
app.get("/", (c) => {
  return c.json({
    ...
    description: "OpenAI Codex App Server REST API (WebSocket JSON-RPC)",
    version: "1.0.0",
    endpoints: [
...
app.get("/models", (c) => {
  return c.json({
    ...
    models: [
      { id: "o4-mini", name: "o4-mini", description: "Default — fast and capable reasoning model" },
      { id: "o3", name: "o3", description: "Strongest — best for complex tasks" },
      { id: "gpt-4.1", name: "GPT-4.1", description: "Balanced — large context, good coding" },
    ],
  })
})
...
app.post("/query", async (c) => {
  ...
  console.log(`[query] done: ${result.duration_ms}ms`)
...
app.post("/session/:id/message", async (c) => {
  ...
  updateSession(id, {
    status: "idle",
    codexSessionId: result.session_id || session.codexSessionId,
  ...
  console.log(`[session:${id}] done: ${result.duration_ms}ms`)
...
console.log("Codex App Server v1.0 (WebSocket JSON-RPC)")
console.log("- Port:", port)
console.log("- App Server:", `ws://127.0.0.1:${process.env.CODEX_APPSERVER_PORT ?? 4500}`)
