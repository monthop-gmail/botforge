...
**คำเตือนนี้ใช้กับทุกตัวเลขในไฟล์นี้** รวมถึงผลของ MCP ข้างล่างด้วย

วัดความผันผวนตรง ๆ แล้ว 2 ตัว ทั้งคู่ให้ผลไม่ซ้ำเดิมทั้งที่สภาพแวดล้อมเหมือนกันหมด:

| model | รอบที่ยิง | ผล |
| --- | ---: | --- |
| `gateway/oc/minimax-m3` | 2 | 1 ครั้งใช้ `webfetch` · 1 ครั้งบอกว่าทำไม่ได้ |
| `thaillm/openthaigpt-8b` | 4 | 1 ครั้งใช้ `webfetch` · 3 ครั้งบอกว่าทำไม่ได้ |
...
> ⚠️ **ตัวเลขนี้มีความไม่แน่นอนแฝงอยู่** — ทุกช่องเป็นผล 1 รอบ และเรารู้แล้วว่า model
> ให้ผลไม่ซ้ำเดิม ส่วนหนึ่งของ 9 ตัวที่พลิกอาจพลิกเองอยู่แล้วโดยไม่เกี่ยวกับ MCP
>
> แต่ **9 จาก 15 ตัว (60%) พลิกพร้อมกัน** ใหญ่เกินกว่าจะอธิบายด้วยความผันผวนล้วน ๆ
> ทิศทางน่าจะถูก แต่ **ขนาดของผลอาจไม่ใหญ่เท่าที่ตัวเลขบอก**
> ถ้าอยากได้ตัวเลขที่เชื่อได้จริงต้องยิง 3–5 รอบต่อ model ทั้งสองสภาพ
...
- ~~`thaillm/openthaigpt-8b` เปลี่ยนเป็น "แต่ง" ตอนมี MCP~~ **ตรวจแล้ว — เป็นความผันผวน**
  ยิงซ้ำ 4 รอบตอนไม่มี MCP: ใช้ `webfetch` 1 · บอกตรง 3 · **ไม่แต่งเลยสักรอบ**
  แปลว่ามันรู้วิธีอยู่แล้วแค่ไม่ได้ทำทุกครั้ง และไม่มีหลักฐานว่า MCP ทำให้แย่ลง
  (แต่ก็ยืนยันไม่ได้ว่าไม่ทำ เพราะ 4 รอบนี้ยิงตอนไม่มี MCP)
