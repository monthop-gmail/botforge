...
app.get("/", (c) => {
  return c.json({
    ...
    description: "GitHub Copilot REST API Server (Copilot SDK)",
    version: "1.0.0",
    endpoints: [
...
app.get("/models", (c) => {
  return c.json({
    ...
    models: [
      { id: "claude-sonnet-4.5", name: "Claude Sonnet 4.5", description: "Default — balanced" },
      { id: "claude-sonnet-4", name: "Claude Sonnet 4", description: "Fast and capable" },
      { id: "gpt-5", name: "GPT-5", description: "OpenAI's latest" },
      { id: "o4-mini", name: "o4-mini", description: "OpenAI reasoning model" },
    ],
  })
})
...
app.post("/query", async (c) => {
  ...
  const { prompt, model, system_prompt, max_turns } = body
...
console.log("Copilot CLI Server v1.0 (Copilot SDK)")
