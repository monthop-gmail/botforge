...
app.get("/", (c) => {
  return c.json({
    ...
    description: "Gemini CLI REST API Server",
    version: "1.0.0",
    endpoints: [
...
app.get("/models", (c) => {
  return c.json({
    ...
    models: [
      { id: "gemini-2.5-flash", name: "Gemini 2.5 Flash", description: "Balanced — good for most tasks" },
      { id: "gemini-2.5-pro", name: "Gemini 2.5 Pro", description: "Strongest — best quality" },
      { id: "gemini-2.0-flash", name: "Gemini 2.0 Flash", description: "Fastest — cheapest" },
    ],
  })
})
...
app.post("/session/:id/message", async (c) => {
  ...
  const result = await runGemini(prompt, {
    model,
    resumeSessionId: session.geminiSessionId ?? undefined,
  ...
  updateSession(id, {
    status: "idle",
    geminiSessionId: result.session_id || session.geminiSessionId,
...
console.log("Gemini CLI Server v1.0")
