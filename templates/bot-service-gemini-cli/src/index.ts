...
console.log("Gemini CLI LINE bot configuration:")
...
function isBotMentioned(event: any): boolean {
  ...
  if (text.startsWith("@bot") || text.startsWith("gemini") || text.startsWith("@gemini")) return true
...
async function handleTextMessage(
  ...
  if (text.toLowerCase() === "/about" || text.toLowerCase() === "/who") {
    const aboutMsg = `สวัสดีครับ! ผมคือ Gemini CLI Bot
...
Bun.serve({
  ...
  async fetch(req: Request) {
    ...
    if (req.method === "GET" && url.pathname === "/") {
      return new Response("Gemini CLI LINE Bot is running")
    }
...
<title>Gemini CLI LINE Bot — About</title>
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  h2{font-size:1.3rem;margin-top:2rem;margin-bottom:.75rem;color:#4285f4}
...
<body>
<div class="container">
  <h1>Gemini CLI LINE Bot</h1>
  <p>AI coding assistant powered by Gemini CLI — ถามคำถาม ให้ AI ช่วยตอบได้เลยผ่าน LINE</p>
  ...
  <footer><p>Built with Gemini CLI + LINE Messaging API</p></footer>
</div>
</body>
</html>`
...
log(
  `Gemini CLI LINE bot listening on http://localhost:${port}/webhook`,
)
