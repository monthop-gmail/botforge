...
app.get("/", (c) => {
  return c.json({
    name: "qwen-code-server",
    description: "Qwen Code CLI REST API Server",
    version: "1.0.0",
    endpoints: [
...
app.post("/session/:id/message", async (c) => {
  ...
  const result = await runQwen(prompt, {
    model,
    resumeSessionId: session.qwenSessionId ?? undefined,
  ...
  updateSession(id, {
    status: "idle",
    qwenSessionId: result.session_id || session.qwenSessionId,
...
console.log("Qwen Code Server v1.0")
