...
{{PROJECT_NAME}} — bridges LINE Messaging API to Claude AI via OpenCode server as middleware. Three Docker services: LINE bot (Bun), OpenCode server (Alpine, includes odoo-mcp), Cloudflare tunnel.
...
# Logs
docker logs {{CONTAINER_PREFIX}}-line-bot --tail 30    # LINE bot logs
docker logs {{CONTAINER_PREFIX}}-server --tail 30      # OpenCode server logs
docker logs {{CONTAINER_PREFIX}}-tunnel                # Tunnel logs

# Useful checks
docker exec {{CONTAINER_PREFIX}}-server cat /root/.local/state/opencode/model.json   # Check default model
docker exec {{CONTAINER_PREFIX}}-server cat /root/.config/opencode/opencode.json     # Check Claude provider config
```
...
- **`{{CONTAINER_PREFIX}}-data`** → `/root/.local/share/opencode` — auth.json (provider tokens)
- **`{{CONTAINER_PREFIX}}-state`** → `/root/.local/state/opencode` — model.json (default model)
